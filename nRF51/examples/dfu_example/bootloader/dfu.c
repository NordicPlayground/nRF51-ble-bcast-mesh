...
{
    nrf_flash_store(m_current_transfer.p_current_page, mp_page_buffer, PAGE_SIZE, 0);
...
{
    dfu_init();
    uint32_t* p_end_addr = SEGMENT_ADDR(segment_count, p_start_addr);
    ...
    else /* double bank */
    {
        ...
        journal_invalidate(PAGE_ALIGN(p_bank_addr));
        nrf_flash_erase(PAGE_ALIGN(p_bank_addr), page_count * PAGE_SIZE); /* This breaks the app */
...
uint32_t dfu_data(uint32_t p_addr, uint8_t* p_data, uint16_t length)
{
    ...
    else
    {
        ...
        nrf_flash_store(p_addr, p_data, length);
